...
Builds a clean, printable system report for a workspace: an operating summary,
per-channel statistics over the stored history, the calibrated parameters in
force, recent alert events, and the node roster. Uses reportlab, which is an
optional extra (``pip install terra-engine[reports]``) so the core stays
dependency-free; the server returns a helpful hint if it's missing.

The caller assembles a plain ``ctx`` dict (see ``build_pdf``) so this module has
no coupling to the server or database.
"""
...
def build_pdf(ctx: dict) -> bytes:
    ...
    story.append(Paragraph("Generated by Terra · from the mud to the moon.", small))
